...
| Manager-SubmitContract-11      | Core       | Submit a Contract with a Signature signed using an RSA algorithm                                          |
| Manager-SubmitContract-12      | Core       | Submit a Contract with a Signature signed using an ECDSA algorithm                                        |
...
| Manager-GetToken-6             | Core       | Request access token with a valid Contract containing a DelegatedServiceConnectionGrant                   |
...
Scenario: Submit a Contract with a Signature signed using an RSA algorithm
When a Contract is submitted by the Peer
Then the Manager should return a 201 and the Contract should be created 

# Manager-SubmitContract-12

Scenario: Submit a Contract with a Signature signed using an ECDSA algorithm
When a Contract is submitted by the Peer
Then the Manager should return a 201 and the Contract should be created
...
Scenario: Place accept signature signed using an RSA algorithm on a Contract
When the Peer submits an accept signature signed using an RSA algorithm
Then the Manager should return a 201 and the Signature should be added

# Manager-AcceptContract-9

Scenario: Place accept signature signed using an RSA algorithm on a Contract
When the Peer submits an accept signature signed using an ECDSA algorithm
Then the Manager should return a 201 and the Signature should be added
...
Scenario: Request access token with a valid Contract containing properties
Given a valid Contract exists with a ServiceConnectionGrant containing the Peer and properties
When the Peer requests an access token
Then the access token with the properties should be returned


# Manager-GetToken-11

Scenario: Request access token with a valid Contract containing properties
Given a valid Contract exists with a DelegatedServiceConnectionGrant containing the Peer and properties
When the Peer requests an access token
Then the access token with the properties should be returned
